...
class PowerScriptTestCase(unittest.TestCase):
    def test_specified_get_args(self):
        ...
        """
        test_args = ["prog", "-s", "vc1", "-u", "testuser", "-o", "4050", "-p", "password", "-q", "-f", "test.ini", "--dc", "mdc1", "on", "testvm", "testvm2"]
        with mock.patch.object(sys, 'argv', test_args):
            ...
            self.assertEqual(result.operation, 'on', "Operation not set correctly")
            self.assertEqual(result.vmname, ['testvm', 'testvm2'], "VMNames not set correctly")
    ...
    def test_default_get_args(self):
        ...
        with mock.patch.object(sys, 'argv', test_args):
            ...
            self.assertEqual(result.operation, 'query', "Required operation not set correctly")
            ...
            self.assertEqual(result.verbose, True, "Default verbosity not True")
            self.assertEqual(result.vmname, ["vmname"], "Required vmname list not set correctly")
    ...
    def test_main_gentle(self, mock_go, mock_reboot, mock_poweroff, mock_poweron, mock_vm, mock_si):
        """
        ...
        test_args = ["prog", "-s", "vc1", "-p", "password", "-u", "username", "-q", "on", "vmname"]
        with mock.patch.object(sys, 'argv', test_args):
            main()
            mock_poweron.assert_called_once()
        test_args = ["prog", "-s", "vc1", "-p", "password", "-u", "username", "-q", "off", "vmname"]
        with mock.patch.object(sys, 'argv', test_args):
            main()
            mock_poweroff.assert_called_once()
            self.assertNotEqual(repr(mock_poweroff.call_args_list).find("False, False"), -1, 
                "Poweroff not called with graceful options")
        test_args = ["prog", "-s", "vc1", "-p", "password", "-u", "username", "-q", "reboot", "vmname"]
        with mock.patch.object(sys, 'argv', test_args):
            main()
            mock_reboot.assert_called_once()
            self.assertNotEqual(repr(mock_reboot.call_args_list).find("False, False"), -1, 
    ...
    @mock.patch('power.vsphere_tools.vm_reboot')
    @mock.patch('power.vsphere_tools.get_obj')
    def test_main_force(self, mock_go, mock_reboot, mock_poweroff, mock_poweron, mock_vm, mock_si):
        """
        ...
        testvm.runtime.powerState.return_value = True
        mock_go.return_value = testvm
        test_args = ["prog", "-s", "vc1", "-p", "password", "-u", "username", "-q", "off", "vmname", "--force"]
        with mock.patch.object(sys, 'argv', test_args):
            main()
            mock_poweroff.assert_called_once()
            self.assertNotEqual(repr(mock_poweroff.call_args_list).find("True, False"), -1, 
                "Poweroff not called with forceful options")
        test_args = ["prog", "-s", "vc1", "-p", "password", "-u", "username", "-q", "reboot", "vmname", "--force"]
        with mock.patch.object(sys, 'argv', test_args):
            main()
            mock_reboot.assert_called_once()
            self.assertNotEqual(repr(mock_reboot.call_args_list).find("True, False"), -1, 
