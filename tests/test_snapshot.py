...
class SnapshotScriptTestCase(unittest.TestCase):
    """
        unittests for snapshot testing
    """
    def test_specified_get_args(self):
        ...
        test_args = ["prog", "-s", "vc1", "-u", "testuser", "-o", "4050",
                     "-p", "password", "-q", "-f", "test.ini", "--dc", "mdc2",
                     "--snapname", "testsnap", "create", "testvm1"]
        with mock.patch.object(sys, 'argv', test_args):
            result = get_args()
            self.assertEqual(result.configfile, 'test.ini',
                             "Configfile not set correctly")
            self.assertEqual(result.dc, 'mdc2', "DC not set correctly")
            self.assertEqual(result.operation, 'create',
                             "Operation not set correctly")
            self.assertEqual(result.vmname, ['testvm1'],
                             "VMname list not set correctly")
            ...
            self.assertEqual(result.snapname, 'testsnap',
                             "Snapname not set correctly")
    ...
    def test_default_get_args(self):
        ...
        with mock.patch.object(sys, 'argv', test_args):
            ...
            self.assertEqual(result.operation, 'create',
                             "Operation is not set correctly")
            self.assertEqual(result.vmname, ['testvm1'],
                             "VMname list not set correctly")
            ...
            self.assertEqual(result.snapname, None,
                             "Default snapname not set correctly")
    ...
    def test_snapshot_main(self, mock_go, mock_deletesnap, mock_revertsnap,
        ...
        test_args = ["prog", "-s", "vc1", "-u", "testuser",
                     "-p", "password", "create", "testvm1",
                     "--snapname", "testsnap", "-q"]
        with mock.patch.object(sys, 'argv', test_args):
            main()
            mock_createsnap.assert_called_once()
        test_args = ["prog", "-s", "vc1", "-u", "testuser",
                     "-p", "password", "delete", "testvm1",
                     "--snapname", "testsnap", "-q"]
        with mock.patch.object(sys, 'argv', test_args):
            main()
            mock_deletesnap.assert_called_once()
        test_args = ["prog", "-s", "vc1", "-u", "testuser",
                     "-p", "password", "revert", "testvm1",
                     "--snapname", "testsnap", "-q"]
        with mock.patch.object(sys, 'argv', test_args):
            main()
            mock_revertsnap.assert_called_once()
        test_args = ["prog", "-s", "vc1", "-u", "testuser",
                     "-p", "password", "list", "testvm1", "-q"]
        with mock.patch.object(sys, 'argv', test_args):
            main()
